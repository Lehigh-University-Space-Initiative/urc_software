...
docker container run \
    ...
    bash -c 'source /opt/ros/humble/setup.sh && colcon build --merge-install --packages-select cross_pkg_messages &&
             ...
             colcon build --merge-install --packages-select base_station_urc main_computer_urc driveline_urc sllidar_ros2 moveit_config_urc arm_urc rover_description_urc'
