...
docker run --rm --name urc_software -it --net=host --ipc=host --pid=host -v /dev:/dev -v /dev/video0:/dev/video0 -v /dev/video1:/dev/video1 --device-cgroup-rule='c 81:* rmw' -e DISPLAY=$DISPLAY --privileged urc_software manual
