...
// Global variables
cross_pkg_messages::msg::RoverComputerDriveCMD currentArmCommand{};
...
int main(int argc, char** argv) {
    ...
    // Create ROS2 node
    node = rclcpp::Node::make_shared("ArmManager");

    RCLCPP_INFO(node->get_logger(), "ArmManager is running");

    // Create the publisher
    armPublisher = node->create_publisher<cross_pkg_messages::msg::RoverComputerArmCMD>("/roverArmCommands", 10);

    // Create the subscriber
    auto subscription = node->create_subscription<cross_pkg_messages::msg::RoverComputerArmCMD>(
        "/armMotorVels", 10, manualInputCallback);
